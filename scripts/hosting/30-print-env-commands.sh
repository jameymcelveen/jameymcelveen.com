...
echo "#   INTERVIEW_API_PROXY_ORIGIN=https://<.NET service>.up.railway.app (required; build-time)"
echo "#   STATS_API_KEY=<same as API>"
echo ""
echo "# --- Railway (variables for Interview API service) ---"
echo "cd backend && railway variables --set \"ANTHROPIC_API_KEY=\$ANTHROPIC_API_KEY\" ..."
echo "# Prefer Railway dashboard → Variables for ANTHROPIC_API_KEY, STATS_API_KEY, connection string, etc."
echo ""
